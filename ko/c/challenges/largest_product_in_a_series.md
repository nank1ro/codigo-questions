...
1000자리 수에서 가장 큰 곱을 가진 네 개의 연속한 숫자는 9 × 9 × 8 × 9 = 5832입니다.

# --instructions--

문자열 `digits`에서 연속하는 `n`개의 숫자가 가장 큰 곱을 갖는 것을 찾아 그 곱을 반환하세요.
...
`largest_product("1234", 2)` 는 `12` 를 반환해야 합니다.

```c
    try_catch(largest_product("1234", 2) == 12);
```

`largest_product("1234", 1)` 은 `4` 를 반환해야 합니다.

```c
    try_catch(largest_product("1234", 1) == 4);
```

`largest_product("99820", 3)` 은 `144` 를 반환해야 합니다.

```c
    try_catch(largest_product("99820", 3) == 144);
```

`largest_product("73167176531330624919225119674426574742355349194934", 4)` 는 `5765760` 을 반환해야 합니다.
