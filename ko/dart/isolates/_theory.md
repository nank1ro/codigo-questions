...
```dart
await Future.delayed(const Duration(seconds: 1)); // waiting: 사용 중인 core가 없음
await Isolate.run(() => hugeCalculation());       // computing: 다른 core가 사용 중
```
...
```dart
final numbers = [1, 2, 3];
await Isolate.run(() => numbers..add(4)); // 복사본이 커짐
print(numbers);                           // [1, 2, 3]
```
