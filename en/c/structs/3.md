...
Complete `area` so it returns the area of the rectangle `r`, that is `width` multiplied by `height`
...
The area of a `3` by `4` rectangle must be `12`
...
The area of a `5` by `5` rectangle must be `25`
...
The area of a `7` by `0` rectangle must be `0`
