...
Complete `free_list` so it frees every node of the list starting at `head`
...
Freeing a list of three nodes must call `free` three times
...
Freeing a single node must call `free` once
...
Freeing an empty list must not crash and must free nothing
