...
Complete `make_node` so it allocates a node on the heap, stores `value` in it, sets `next` to `NULL` and returns its address
...
`make_node(5)` must return a node whose `value` is `5`
...
The `next` of a new node must be `NULL`
...
Two calls must return two different nodes
