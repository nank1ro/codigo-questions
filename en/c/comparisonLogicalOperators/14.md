...
Complete `sign` so it returns `1` when `n` is positive, `-1` when it is negative and `0` when it is zero
...
`sign(9)` must return `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` must return `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` must return `0`
