...
`double_it` does not compile because the trailing comment on the first line is never closed, so it swallows the `return`. Close the comment at the end of that same line
...
`double_it(4)` must return `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` must return `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` must return `0`
