...
`area` always returns `0` because its real computation is commented out. Uncomment the `return w * h;` line and delete the `return 0;` placeholder
...
`area(3, 4)` must return `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` must return `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` must return `0`
