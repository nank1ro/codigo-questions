...
Complete `digit_value` so it returns the number represented by the digit character `c` (for example `'7'` gives `7`)
...
`digit_value('7')` must return `7`

```c
    try_catch(digit_value('7') == 7);
```

`digit_value('0')` must return `0`

```c
    try_catch(digit_value('0') == 0);
```

`digit_value('9')` must return `9`
