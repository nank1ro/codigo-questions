...
Complete `can_add` so it returns `1` when `a + b` does not exceed `INT_MAX` and `0` otherwise (both arguments are non-negative)
...
`can_add(1, 2)` must return `1`

```c
    try_catch(can_add(1, 2) == 1);
```

`can_add(INT_MAX, 1)` must return `0`

```c
    try_catch(can_add(INT_MAX, 1) == 0);
```

`can_add(INT_MAX - 5, 5)` must return `1`
