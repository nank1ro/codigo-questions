...
Complete `truncate` so it returns the whole part of `x` as an `int`, using an explicit cast
...
`truncate(3.99)` must return `3`

```c
    try_catch(truncate(3.99) == 3);
```

`truncate(-2.5)` must return `-2`

```c
    try_catch(truncate(-2.5) == -2);
```

`truncate(7.0)` must return `7`
