...
Complete `to_bool` so it returns `false` when `x` is exactly `0` and `true` for any other value
...
`to_bool(0.5)` must return `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` must return `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` must return `true`
