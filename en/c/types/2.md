...
Complete `to_celsius` so it converts a temperature `f` from Fahrenheit to Celsius using the formula `(f - 32) * 5 / 9`
...
`to_celsius(212)` must return `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` must return `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` must return `10`
