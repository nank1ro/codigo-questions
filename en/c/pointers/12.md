...
Complete `divide` so it stores the integer quotient of `a` by `b` in the variable that `quotient` points to, and the remainder in the one that `remainder` points to
...
`17` divided by `5` must give quotient `3` and remainder `2`
...
`20` divided by `4` must give quotient `5` and remainder `0`
...
`3` divided by `7` must give quotient `0` and remainder `3`
