...
Complete `find_max` so it returns a pointer to the largest of the `size` elements in `values`, or `NULL` when `size` is `0`
...
In `{3, 9, 4}` the result must point to the `9`
...
The value read through the result of `{-2, -8, -1}` must be `-1`
