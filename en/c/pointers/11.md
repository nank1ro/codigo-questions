...
Complete `swap` so it exchanges the values of the two variables that `a` and `b` point to
...
Swapping `1` and `2` must give `2` and `1`
...
Swapping `-5` and `40` must give `40` and `-5`
...
Swapping twice must restore the original values
