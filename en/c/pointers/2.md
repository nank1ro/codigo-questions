...
Complete `sum_at` so it returns the sum of the values that `a` and `b` point to
...
Pointing at `3` and `4` the sum must be `7`
...
Pointing at `10` and `-10` the sum must be `0`
...
Passing the same address twice must double its value
