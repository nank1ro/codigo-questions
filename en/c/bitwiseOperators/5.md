...
Complete `power_of_two` so it returns 2 raised to `n`, using a left shift on `1u` and assuming `n` is smaller than 32
...
`power_of_two(0)` must return `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` must return `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` must return `1024`
