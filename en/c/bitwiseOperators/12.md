...
Complete `is_bit_set` so it returns `1` when bit `n` of `value` is on and `0` otherwise, assuming `n` is smaller than 32
...
`is_bit_set(10, 1)` must return `1`

```c
    try_catch(is_bit_set(10, 1) == 1);
```

`is_bit_set(10, 2)` must return `0`

```c
    try_catch(is_bit_set(10, 2) == 0);
```

`is_bit_set(10, 3)` must return `1`
