...
Complete `count_chars` so it returns the number of characters in `text` using `strlen`
...
`count_chars("hello")` must return `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` must return `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` must return `10`
