...
Write a function `binarySearch` that takes an array of integers sorted in ascending order, its length and a target integer, and returns the index of the target inside the array, or `-1` when the target is not present.
