...
Write a function `monthOf(date: String): String` that returns the month of a date written as `YYYY-MM-DD`, or an empty string when `date` contains no such date.

Example: `monthOf("2026-09-12")` returns `09`.
...
`monthOf("1999-12-31")` must return `12`

```kotlin
    tryCatch(monthOf("1999-12-31") == "12")
```

`monthOf("due 2024-01-05 sharp")` must return `01`
