...
The function `price(item)` returns a `Future<int>` with the price of `item`. Use `await` to get the price of `'apple'` and the price of `'milk'`, and store their sum in `total`.
