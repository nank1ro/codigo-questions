...
Complete `count_bits` so it returns how many bits of `value` are set to `1`
...
`count_bits(0)` must return `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` must return `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` must return `4`
