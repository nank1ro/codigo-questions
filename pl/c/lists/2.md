...
Uzupełnij `make_node`, aby alokowała węzeł na stercie, zapisywała w nim `value`, ustawiała `next` na `NULL` i zwracała jego adres
...
`make_node(5)` musi zwrócić węzeł, którego `value` to `5`
...
`next` nowego węzła musi być równe `NULL`
...
Dwa wywołania muszą zwrócić dwa różne węzły
