...
Uzupełnij `count_chars`, aby zwracała liczbę znaków w `text`, używając `strlen`
...
`count_chars("hello")` musi zwrócić `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` musi zwrócić `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` musi zwrócić `10`
