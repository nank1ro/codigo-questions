...
Uzupełnij `sum_between`, aby zwracała sumę wszystkich elementów od adresu `start` aż do adresu `end`, ale bez niego
...
Pierwsze trzy elementy `{1, 2, 3, 4}` muszą sumować się do `6`
...
Elementy od indeksu `1` do `3` z `{5, 6, 7, 8}` muszą sumować się do `21`
...
Pusty zakres musi sumować się do `0`
