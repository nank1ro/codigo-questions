...
Uzupełnij `find_max`, aby zwracała wskaźnik na największy z `size` elementów w `values`, albo `NULL`, gdy `size` wynosi `0`
...
Dla `{3, 9, 4}` wynik musi wskazywać na `9`
...
Wartość odczytana przez wynik dla `{-2, -8, -1}` musi wynosić `-1`
