...
Wykonaj pracę opisaną w komentarzu `TODO` w `is_even`, a następnie usuń komentarz, ponieważ praca jest gotowa
...
```c
int is_even(int n) {
    // TODO: zwróć 1, gdy n jest parzyste, w przeciwnym razie 0
    return 0;
}
```
...
`is_even(4)` musi zwrócić `1`

```c
    try_catch(is_even(4) == 1);
```

`is_even(7)` musi zwrócić `0`

```c
    try_catch(is_even(7) == 0);
```

`is_even(0)` musi zwrócić `1`
