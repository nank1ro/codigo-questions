...
Funkcja `double_it` się nie kompiluje, ponieważ komentarz końcowy w pierwszej linii nigdy nie zostaje zamknięty, więc pochłania `return`. Zamknij komentarz na końcu tej samej linii
...
`double_it(4)` musi zwrócić `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` musi zwrócić `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` musi zwrócić `0`
