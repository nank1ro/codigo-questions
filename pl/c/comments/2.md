...
Funkcja `area` zawsze zwraca `0`, ponieważ jej prawdziwe obliczenie jest zakomentowane. Odkomentuj linię `return w * h;` i usuń zastępczą linię `return 0;`
...
`area(3, 4)` musi zwrócić `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` musi zwrócić `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` musi zwrócić `0`
