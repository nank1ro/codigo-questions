...
Uzupełnij `count_bits`, aby zwracała, ile bitów `value` jest ustawionych na `1`
...
`count_bits(0)` musi zwrócić `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` musi zwrócić `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` musi zwrócić `4`
