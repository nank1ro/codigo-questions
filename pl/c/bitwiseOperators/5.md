...
Uzupełnij `power_of_two`, aby zwracała 2 do potęgi `n`, używając przesunięcia w lewo na `1u` i zakładając, że `n` jest mniejsze od 32
...
`power_of_two(0)` musi zwrócić `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` musi zwrócić `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` musi zwrócić `1024`
