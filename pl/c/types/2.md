...
Uzupełnij `to_celsius`, aby konwertowała temperaturę `f` z Fahrenheita na Celsjusza według wzoru `(f - 32) * 5 / 9`
...
`to_celsius(212)` musi zwrócić `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` musi zwrócić `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` musi zwrócić `10`
