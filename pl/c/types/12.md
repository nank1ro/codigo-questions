...
Uzupełnij `can_add`, aby zwracała `1`, gdy `a + b` nie przekracza `INT_MAX`, a w przeciwnym razie `0` (oba argumenty są nieujemne)
...
`can_add(1, 2)` musi zwrócić `1`

```c
    try_catch(can_add(1, 2) == 1);
```

`can_add(INT_MAX, 1)` musi zwrócić `0`

```c
    try_catch(can_add(INT_MAX, 1) == 0);
```

`can_add(INT_MAX - 5, 5)` musi zwrócić `1`
