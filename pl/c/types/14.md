...
Uzupełnij `to_bool`, aby zwracała `false`, gdy `x` jest dokładnie `0`, i `true` dla każdej innej wartości
...
`to_bool(0.5)` musi zwrócić `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` musi zwrócić `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` musi zwrócić `true`
