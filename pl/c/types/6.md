...
Uzupełnij `truncate`, aby zwracała część całkowitą `x` jako `int`, używając jawnego rzutowania
...
`truncate(3.99)` musi zwrócić `3`

```c
    try_catch(truncate(3.99) == 3);
```

`truncate(-2.5)` musi zwrócić `-2`

```c
    try_catch(truncate(-2.5) == -2);
```

`truncate(7.0)` musi zwrócić `7`
