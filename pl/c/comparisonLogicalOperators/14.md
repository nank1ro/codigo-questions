...
Uzupełnij `sign`, aby zwracała `1`, gdy `n` jest dodatnie, `-1`, gdy jest ujemne, i `0`, gdy wynosi zero
...
`sign(9)` musi zwrócić `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` musi zwrócić `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` musi zwrócić `0`
