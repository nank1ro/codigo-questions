...
Uzupełnij `is_even`, aby zwracała `true`, gdy `n` jest parzyste, a w przeciwnym razie `false`
...
`is_even(4)` musi zwrócić `true`

```c
    try_catch(is_even(4) == true);
```

`is_even(7)` musi zwrócić `false`

```c
    try_catch(is_even(7) == false);
```

`is_even(0)` musi zwrócić `true`
