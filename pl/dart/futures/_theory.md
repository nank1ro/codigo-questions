...
Future<int> count() async {
  return 3;                 // int, opakowany w Future<int>
}

Future<void> main() async {
  int n = await count();    // Future<int>, rozpakowany do int
  Future<int> f = count();  // brak await: to nadal Future<int>
}
```
...
Future<int> parseLater(String s) async {
  await Future.delayed(const Duration(milliseconds: 5));
  return int.parse(s); // rzuca FormatException dla 'abc'
}
...
Future<double> average() async {
  final p = pages();          // oba timery startują teraz
  final w = words();
  return await w / await p;   // czeka raz, łącznie około 20 ms
}
```
...
Future<int> loadTwice() async {
  final n = await load();   // rzuca tutaj, loadTwice też zawodzi
  return n * 2;             // nigdy się nie uruchamia
}
...
Future<String> onceThenGiveUp(Future<String> Function() task) async {
  try {
    ...
    rethrow; // wywołujący widzi oryginalny błąd
  }
}
```
