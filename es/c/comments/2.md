...
`area` siempre devuelve `0` porque su cálculo real está comentado. Descomenta la línea `return w * h;` y borra el marcador `return 0;`
...
`area(3, 4)` debe devolver `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` debe devolver `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` debe devolver `0`
