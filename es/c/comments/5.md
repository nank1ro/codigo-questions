...
`double_it` no compila porque el comentario al final de la primera línea nunca se cierra, así que se traga el `return`. Cierra el comentario al final de esa misma línea
...
```c
int double_it(int n) {
    int result = n * 2; /* el doble de la entrada
    return result;
}
```
...
`double_it(4)` debe devolver `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` debe devolver `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` debe devolver `0`
