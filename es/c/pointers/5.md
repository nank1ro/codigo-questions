...
Completa `value_or` para que devuelva el valor al que `p` apunta, o `fallback` cuando `p` es `NULL`
...
Apuntando a `2.5` el resultado debe ser `2.5`
...
Con un puntero `NULL` el resultado debe ser el fallback `-1.0`

```c
    try_catch(value_or(NULL, -1.0) == -1.0);
```

Apuntando a `0.0` el resultado debe ser `0.0`, no el fallback
