...
Completa `find_max` para que devuelva un puntero al mayor de los `size` elementos de `values`, o `NULL` cuando `size` es `0`
...
En `{3, 9, 4}` el resultado debe apuntar al `9`
...
El valor leído a través del resultado de `{-2, -8, -1}` debe ser `-1`
