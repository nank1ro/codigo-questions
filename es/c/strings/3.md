...
Completa `count_chars` para que devuelva el número de caracteres en `text` usando `strlen`
...
`count_chars("hello")` debe devolver `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` debe devolver `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` debe devolver `10`
