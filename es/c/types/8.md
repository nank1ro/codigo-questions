...
Completa `digit_value` para que devuelva el número representado por el carácter de dígito `c` (por ejemplo, `'7'` da `7`)
...
`digit_value('7')` debe devolver `7`

```c
    try_catch(digit_value('7') == 7);
```

`digit_value('0')` debe devolver `0`

```c
    try_catch(digit_value('0') == 0);
```

`digit_value('9')` debe devolver `9`
