...
Completa `to_bool` para que devuelva `false` cuando `x` sea exactamente `0` y `true` para cualquier otro valor
...
`to_bool(0.5)` debe devolver `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` debe devolver `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` debe devolver `true`
