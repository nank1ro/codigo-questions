...
Completa `average` para que devuelva el promedio de `a` y `b` como un `double`, conservando la parte decimal
...
`average(3, 4)` debe devolver `3.5`

```c
    try_catch(average(3, 4) == 3.5);
```

`average(2, 2)` debe devolver `2`
