...
Completa `to_celsius` para que convierta una temperatura `f` de Fahrenheit a Celsius usando la fórmula `(f - 32) * 5 / 9`
...
`to_celsius(212)` debe devolver `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` debe devolver `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` debe devolver `10`
