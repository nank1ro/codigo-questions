...
Completa `can_add` para que devuelva `1` cuando `a + b` no supere `INT_MAX` y `0` en caso contrario (ambos argumentos son no negativos)
...
`can_add(1, 2)` debe devolver `1`

```c
    try_catch(can_add(1, 2) == 1);
```

`can_add(INT_MAX, 1)` debe devolver `0`

```c
    try_catch(can_add(INT_MAX, 1) == 0);
```

`can_add(INT_MAX - 5, 5)` debe devolver `1`
