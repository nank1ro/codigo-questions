...
Completa `low_nibble` para que devuelva solo los cuatro bits más bajos de `value`
...
`low_nibble(0xAB)` debe devolver `11`

```c
    try_catch(low_nibble(0xAB) == 11);
```

`low_nibble(255)` debe devolver `15`

```c
    try_catch(low_nibble(255) == 15);
```

`low_nibble(16)` debe devolver `0`
