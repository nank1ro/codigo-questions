...
Completa `power_of_two` para que devuelva 2 elevado a `n`, usando un desplazamiento a la izquierda de `1u` y suponiendo que `n` es menor que 32
...
`power_of_two(0)` debe devolver `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` debe devolver `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` debe devolver `1024`
