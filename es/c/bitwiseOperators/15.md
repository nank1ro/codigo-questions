...
Completa `count_bits` para que devuelva cuántos bits de `value` están puestos a `1`
...
`count_bits(0)` debe devolver `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` debe devolver `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` debe devolver `4`
