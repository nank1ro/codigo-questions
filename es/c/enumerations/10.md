...
Completa `next_color` para que devuelva la constante siguiente a `color`, volviendo de `BLUE` a `RED`. Hay 3 colores, así que usa `% 3`
...
`next_color(RED)` debe devolver `GREEN`

```c
    try_catch(next_color(RED) == GREEN);
```

`next_color(GREEN)` debe devolver `BLUE`

```c
    try_catch(next_color(GREEN) == BLUE);
```

`next_color(BLUE)` debe devolver `RED`
