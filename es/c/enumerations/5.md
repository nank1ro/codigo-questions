...
Completa `wheels` para que devuelva `2` para `BIKE`, `4` para `CAR` y `6` para `TRUCK`, usando un `switch` sobre `vehicle`
...
`wheels(BIKE)` debe devolver `2`

```c
    try_catch(wheels(BIKE) == 2);
```

`wheels(CAR)` debe devolver `4`

```c
    try_catch(wheels(CAR) == 4);
```

`wheels(TRUCK)` debe devolver `6`
