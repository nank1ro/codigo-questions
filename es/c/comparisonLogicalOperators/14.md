...
Completa `sign` para que devuelva `1` cuando `n` sea positivo, `-1` cuando sea negativo y `0` cuando sea cero
...
`sign(9)` debe devolver `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` debe devolver `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` debe devolver `0`
