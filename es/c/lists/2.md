...
Completa `make_node` para que asigne un nodo en el heap, guarde `value` en él, ponga `next` a `NULL` y devuelva su dirección
...
`make_node(5)` debe devolver un nodo cuyo `value` sea `5`
...
El `next` de un nodo nuevo debe ser `NULL`
...
Dos llamadas deben devolver dos nodos distintos
