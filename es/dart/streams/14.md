---
language: dart
exerciseType: 2
---

# --description--

Un controlador puede crear un stream broadcast directamente con el constructor con nombre `StreamController<T>.broadcast()`. Su `stream` acepta cualquier número de oyentes, y cada evento se entrega a todos ellos, en el orden en que se suscribieron:

```dart
import 'dart:async';

void main() {
  final controller = StreamController<String>.broadcast();
  controller.stream.listen((msg) => print('first: $msg'));
  controller.stream.listen((msg) => print('second: $msg'));
  controller.add('hi');
  controller.close();
}
// first: hi
// second: hi
```

Como todo stream broadcast, no usa búfer: los eventos añadidos antes de que un oyente se suscriba se pierden para ese oyente.

# --instructions--

Completa el código para crear un controlador broadcast, suscribir dos oyentes `A` y `B`, enviar los valores `1` y `2`, y luego cerrar el controlador, de modo que el programa imprima `A got 1`, `B got 1`, `A got 2`, `B got 2`.

# --seed--

```dart
import 'dart:async';

void main() {
  final controller = StreamController<int>.[/]();
  controller.stream.[/]((n) => print('A got $n'));
  controller.stream.listen((n) => print('B got $n'));
  controller.[/](1);
  controller.add(2);
  controller.[/]();
}
```

# --answers--

- broadcast
- listen
- add
- close
- asBroadcastStream
- subscribe
- sink

# --solutions--

```dart
import 'dart:async';

void main() {
  final controller = StreamController<int>.broadcast();
  controller.stream.listen((n) => print('A got $n'));
  controller.stream.listen((n) => print('B got $n'));
  controller.add(1);
  controller.add(2);
  controller.close();
}
```

# --output--

A got 1
B got 1
A got 2
B got 2
