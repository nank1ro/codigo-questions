...
For every en/$L/$A/<n>.md create $LOC/$L/$A/<n>.md, a faithful translation into the language of locale '$LOC'. Frontmatter, before-seed, after-seed, before-asserts, assert code blocks, after-asserts and output must be BYTE-IDENTICAL to en; the learner never sees those, so their comments stay in English. In the sections the learner reads (description, instructions, seed, type-2 '# --answers--' tokens and solutions) keep the code itself identical to en - same identifiers, literals, structure and '[/]' placeholders - but DO translate the comments inside the code blocks. A comment appearing in more than one of seed, answers and solutions must get the SAME translation in each, so the solution still matches the seed and the answer tokens exactly. Translate: description, instructions, the prose line above each assert code block, type-3 answers and type-3 solutions (the translated solution must equal one translated answer exactly). Keep markdown structure, section order, code spans, '[/]' placeholders, '--err-tN--' markers and blank-line layout identical to en. Translate complete sentences; leave no English prose behind. Match the tone and terminology of existing files in $LOC/$L/.
