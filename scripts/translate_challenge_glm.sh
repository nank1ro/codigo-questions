...
For each language create $LOC/<language>/challenges/$CH.md, a faithful translation into the language of locale '$LOC'. Frontmatter (except the 'title:' line), before-seed, after-seed, before-asserts, assert code blocks, after-asserts and output must be BYTE-IDENTICAL to en; the learner never sees those, so their comments stay in English. In the sections the learner reads (description, instructions, seed and solutions) keep the code itself identical to en - same identifiers, literals and structure - but DO translate the comments inside the code blocks, using the SAME translation for a comment that appears in more than one section. The frontmatter 'title:' value must be translated into the language of locale '$LOC' and must be EXACTLY the same string used for the \"name\" of the \"$CH\" entry in that language's $LOC/<language>/challenges/data.json. Translate: description, instructions and the prose line above each assert code block. Keep markdown structure, section order, code spans, '--err-tN--' markers and blank-line layout identical to en. Translate complete sentences; leave no English prose behind. Match the tone and terminology of the existing translated challenges already in $LOC/<language>/challenges/.
