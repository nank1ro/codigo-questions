...
```swift
let retries = 3 // nach drei Versuchen aufgeben
```
...
```swift
let width = 10 /* in Zentimetern
print(width) // noch innerhalb des Kommentars: Fehler, der Kommentar wird nie geschlossen
```
...
```swift
/// Gibt `true` zurück, wenn `n` durch zwei teilbar ist.
