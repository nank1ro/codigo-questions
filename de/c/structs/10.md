...
Vervollständige `deposit`, sodass `amount` zum `balance` des Kontos addiert wird, auf das `account` zeigt
...
Eine Einzahlung von `50` auf einen Stand von `100` muss `150` ergeben
...
Eine Einzahlung von `0` muss den Kontostand unverändert lassen
...
Zwei Einzahlungen von `10` und `20` auf ein leeres Konto müssen `30` ergeben
