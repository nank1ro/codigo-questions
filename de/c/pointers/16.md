...
Vervollständige `find_max`, sodass die Funktion einen Zeiger auf das größte der `size` Elemente in `values` zurückgibt, oder `NULL`, wenn `size` `0` ist
...
In `{3, 9, 4}` muss das Ergebnis auf die `9` zeigen
...
Der über das Ergebnis von `{-2, -8, -1}` gelesene Wert muss `-1` sein
