...
Vervollständige `swap`, sodass die Funktion die Werte der beiden Variablen vertauscht, auf die `a` und `b` zeigen
...
Das Vertauschen von `1` und `2` muss `2` und `1` ergeben
...
Das Vertauschen von `-5` und `40` muss `40` und `-5` ergeben
...
Doppeltes Vertauschen muss die ursprünglichen Werte wiederherstellen
