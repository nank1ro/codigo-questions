...
Vervollständige `sum_between`, sodass die Funktion die Summe aller Elemente ab der Adresse `start` bis zur Adresse `end` zurückgibt, diese aber nicht einschließend
...
Die ersten drei Elemente von `{1, 2, 3, 4}` müssen sich zu `6` summieren
...
Die Elemente von Index `1` bis `3` von `{5, 6, 7, 8}` müssen sich zu `21` summieren
...
Ein leerer Bereich muss sich zu `0` summieren
