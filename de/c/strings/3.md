...
Vervollständige `count_chars`, sodass die Funktion mithilfe von `strlen` die Anzahl der Zeichen in `text` zurückgibt
...
`count_chars("hello")` sollte `5` zurückgeben

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` sollte `0` zurückgeben

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` sollte `10` zurückgeben
