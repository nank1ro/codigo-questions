...
Vervollständige `power_of_two`, sodass die Funktion 2 hoch `n` zurückgibt, indem sie `1u` nach links schiebt, und nimm an, dass `n` kleiner als 32 ist
...
`power_of_two(0)` sollte `1` zurückgeben

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` sollte `8` zurückgeben

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` sollte `1024` zurückgeben
