...
Vervollständige `is_bit_set`, sodass die Funktion `1` zurückgibt, wenn Bit `n` von `value` an ist, und sonst `0`, und nimm an, dass `n` kleiner als 32 ist
...
`is_bit_set(10, 1)` sollte `1` zurückgeben

```c
    try_catch(is_bit_set(10, 1) == 1);
```

`is_bit_set(10, 2)` sollte `0` zurückgeben

```c
    try_catch(is_bit_set(10, 2) == 0);
```

`is_bit_set(10, 3)` sollte `1` zurückgeben
