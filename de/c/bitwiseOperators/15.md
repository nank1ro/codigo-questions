...
Vervollständige `count_bits`, sodass die Funktion zurückgibt, wie viele Bits von `value` auf `1` gesetzt sind
...
`count_bits(0)` sollte `0` zurückgeben

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` sollte `8` zurückgeben

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` sollte `4` zurückgeben
