...
Vervollständige `sign`, sodass die Funktion `1` zurückgibt, wenn `n` positiv ist, `-1`, wenn es negativ ist, und `0`, wenn es null ist
...
`sign(9)` muss `1` zurückgeben

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` muss `-1` zurückgeben

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` muss `0` zurückgeben
