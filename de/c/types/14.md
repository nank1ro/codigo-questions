...
Vervollständige `to_bool` so, dass es `false` zurückgibt, wenn `x` genau `0` ist, und `true` für jeden anderen Wert
...
`to_bool(0.5)` muss `true` zurückgeben

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` muss `false` zurückgeben

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` muss `true` zurückgeben
