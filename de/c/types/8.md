...
Vervollständige `digit_value` so, dass es die Zahl zurückgibt, die durch das Ziffernzeichen `c` dargestellt wird (zum Beispiel ergibt `'7'` die Zahl `7`)
...
`digit_value('7')` muss `7` zurückgeben

```c
    try_catch(digit_value('7') == 7);
```

`digit_value('0')` muss `0` zurückgeben

```c
    try_catch(digit_value('0') == 0);
```

`digit_value('9')` muss `9` zurückgeben
