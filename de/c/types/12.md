...
Vervollständige `can_add` so, dass es `1` zurückgibt, wenn `a + b` `INT_MAX` nicht überschreitet, und sonst `0` (beide Argumente sind nicht negativ)
...
`can_add(1, 2)` muss `1` zurückgeben

```c
    try_catch(can_add(1, 2) == 1);
```

`can_add(INT_MAX, 1)` muss `0` zurückgeben

```c
    try_catch(can_add(INT_MAX, 1) == 0);
```

`can_add(INT_MAX - 5, 5)` muss `1` zurückgeben
