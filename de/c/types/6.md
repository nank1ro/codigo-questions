...
Vervollständige `truncate` so, dass es den ganzzahligen Teil von `x` als `int` zurückgibt, mithilfe einer expliziten Umwandlung
...
`truncate(3.99)` muss `3` zurückgeben

```c
    try_catch(truncate(3.99) == 3);
```

`truncate(-2.5)` muss `-2` zurückgeben

```c
    try_catch(truncate(-2.5) == -2);
```

`truncate(7.0)` muss `7` zurückgeben
