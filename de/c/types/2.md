...
Vervollständige `to_celsius` so, dass es eine Temperatur `f` von Fahrenheit in Celsius umrechnet, mit der Formel `(f - 32) * 5 / 9`
...
`to_celsius(212)` muss `100` zurückgeben

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` muss `0` zurückgeben

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` muss `10` zurückgeben
