...
Vervollständige `average` so, dass es den Durchschnitt von `a` und `b` als `double` zurückgibt und dabei den Dezimalteil behält
...
`average(3, 4)` muss `3.5` zurückgeben

```c
    try_catch(average(3, 4) == 3.5);
```

`average(2, 2)` muss `2` zurückgeben
