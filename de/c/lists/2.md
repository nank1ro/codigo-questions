...
Vervollständige `make_node`, sodass es einen Knoten auf dem Heap alloziert, `value` darin speichert, `next` auf `NULL` setzt und seine Adresse zurückgibt
...
`make_node(5)` muss einen Knoten zurückgeben, dessen `value` `5` ist
...
Das `next` eines neuen Knotens muss `NULL` sein
...
Zwei Aufrufe müssen zwei verschiedene Knoten zurückgeben
