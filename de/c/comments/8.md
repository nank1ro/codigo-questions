...
Vervollständige die Arbeit, die der `TODO`-Kommentar in `is_even` beschreibt, und entferne anschließend den Kommentar, da die Arbeit erledigt ist
...
```c
int is_even(int n) {
    // TODO: gib 1 zurück, wenn n gerade ist, sonst 0
    return 0;
}
```
...
`is_even(4)` muss `1` zurückgeben

```c
    try_catch(is_even(4) == 1);
```

`is_even(7)` muss `0` zurückgeben

```c
    try_catch(is_even(7) == 0);
```

`is_even(0)` muss `1` zurückgeben
