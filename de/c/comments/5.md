...
`double_it` kompiliert nicht, weil der nachgestellte Kommentar in der ersten Zeile nie geschlossen wird und deshalb das `return` verschluckt. Schließe den Kommentar am Ende derselben Zeile
...
`double_it(4)` muss `8` zurückgeben

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` muss `-6` zurückgeben

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` muss `0` zurückgeben
