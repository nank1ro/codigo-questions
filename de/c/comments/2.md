...
`area` gibt immer `0` zurück, weil die eigentliche Berechnung auskommentiert ist. Entkommentiere die Zeile `return w * h;` und lösche den Platzhalter `return 0;`
...
`area(3, 4)` muss `12` zurückgeben

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` muss `25` zurückgeben

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` muss `0` zurückgeben
