...
- Wenn die Zahl ein Vielfaches von beiden `3` und `5` ist, sollte die Ausgabe `"FizzBuzz"` sein.
- Wenn die Zahl weder ein Vielfaches von `3` noch von `5` ist, sollte die Zahl selbst ausgegeben werden, wie in den Beispielen unten gezeigt.
...
char* fizz_buzz(int number) {
    ...
    char* string_number = malloc(12); // Unter der Annahme von maximal 11 Zeichen für die Zahl
