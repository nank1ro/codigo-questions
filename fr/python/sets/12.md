---
language: python
exerciseType: 1
---

# --description--

Un set ne peut contenir que des éléments **hachables**, c'est-à-dire des valeurs qui ne peuvent pas changer : des nombres, des chaînes, `True`/`False` et des **tuples**.
Essayer d'ajouter une liste, un dictionnaire ou un autre set lève une `TypeError` :
```python
points = set()
points.add((1, 2))  # ok, un tuple
points.add([1, 2])  # TypeError: unhashable type: 'list'
```
Les sets de tuples sont pratiques pour garder trace de paires uniques, comme des coordonnées ou des enregistrements (nom, âge) :
```python
visits = [(1, 2), (1, 2), (3, 4)]
print(len(set(visits)))  # 2
```

# --instructions--

Stockez dans `points` le set des coordonnées distinctes de `visits`, ajoutez-lui le point `(0, 0)`, puis stockez dans `count` le nombre de points distincts qu'il contient

# --seed--

```python
visits = [(1, 2), (3, 4), (1, 2), (5, 6), (3, 4)]
```

# --before-asserts--

```python
import unittest

class CodigoTests(unittest.TestCase):
```

# --asserts--

Vous devez utiliser `add()`

```json
{
  "regex": "\\.add\\(",
  "modifiers": ["m"],
  "shouldMatch": true
}
```

`points` doit être égal à `{(0, 0), (1, 2), (3, 4), (5, 6)}`

```python
    def test_1(self):
        self.assertEqual(points, {(0, 0), (1, 2), (3, 4), (5, 6)}, "--err-t1--")
```

`count` doit être égal à `4`

```python
    def test_2(self):
        self.assertEqual(count, 4, "--err-t2--")
```

# --after-asserts--

```python
if __name__ == "__main__":
    unittest.main()
```

# --solutions--

```python
visits = [(1, 2), (3, 4), (1, 2), (5, 6), (3, 4)]
points = set(visits)
points.add((0, 0))
count = len(points)
```
