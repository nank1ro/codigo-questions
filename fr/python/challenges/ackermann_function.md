---
language: python
exerciseType: 1
title: Fonction d'Ackermann
difficulty: 1
---

# --description--

La fonction d'Ackermann est un exemple classique d'une fonction récursive, notable en particulier parce qu'elle n'est pas une fonction primitive récursive. Elle croît très rapidement en valeur, tout comme la taille de son arbre d'appels.

La fonction d'Ackermann est généralement définie comme suit:

<latex>A(m, n) = \begin{cases} n + 1 &\text{if } m = 0 \\ A(m - 1,1) &\text{if } m > 0 \text{ and } n = 0 \\ A(m -1, A(m, n - 1)) &\text{if } m > 0 \text{ and } n > 0 \end{cases}</latex>

Ses arguments ne sont jamais négatifs et elle se termine toujours

# --instructions--

Écrivez une fonction qui retourne la valeur de la fonction d'Ackermann.

# --seed--

```python
def ack(m, n):
    pass
```

# --before-asserts--

```python
import unittest

class CodigoTests(unittest.TestCase):
```

# --asserts--

`ack(0, 0)` doit retourner 1.

```python
    def test1(self):
        self.assertEqual(ack(0, 0), 1, "--err-t1--")
```

`ack(1, 1)` doit retourner 3.

```python
    def test2(self):
        self.assertEqual(ack(1, 1), 3, "--err-t2--")
```

`ack(2, 5)` doit retourner 13.

```python
    def test3(self):
        self.assertEqual(ack(2, 5), 13, "--err-t3--")
```

`ack(3, 3)` doit retourner 61.

```python
    def test4(self):
        self.assertEqual(ack(3, 3), 61, "--err-t4--")
```

# --after-asserts--

```python
if __name__ == "__main__":
    unittest.main()
```

# --solutions--

```python
def ack(m, n):
    if m == 0:
        return n + 1
    else:
        if (n == 0):
            return ack(m - 1, 1)
        return ack(m - 1, ack(m, n - 1))
```
