...
La somme des nombres premiers en dessous de 10 est 2 + 3 + 5 + 7 = 17.
...
class PrimeSeive {
  constructor(num) {
    ...
    for (let i = 0; i <= sqrtUpper; i++) {
      if (seive[i]) {
        // Marque la valeur dans le tableau seive
        const prime = 2 * i + 3;
        // Marque tous les multiples de ce nombre comme false (non premiers)
