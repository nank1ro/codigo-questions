...
Complétez `sign` pour qu'elle retourne `1` quand `n` est positif, `-1` quand il est négatif et `0` quand il est nul
...
`sign(9)` doit retourner `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` doit retourner `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` doit retourner `0`
