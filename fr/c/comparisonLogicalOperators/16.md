...
Complétez `is_even` pour qu'elle retourne `true` quand `n` est pair et `false` sinon
...
`is_even(4)` doit retourner `true`

```c
    try_catch(is_even(4) == true);
```

`is_even(7)` doit retourner `false`

```c
    try_catch(is_even(7) == false);
```

`is_even(0)` doit retourner `true`
