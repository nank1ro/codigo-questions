...
Complétez `power_of_two` pour qu'elle retourne 2 élevé à la puissance `n`, en utilisant un décalage à gauche sur `1u` et en supposant que `n` est plus petit que 32
...
`power_of_two(0)` doit retourner `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` doit retourner `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` doit retourner `1024`
