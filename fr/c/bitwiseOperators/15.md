...
Complétez `count_bits` pour qu'elle retourne le nombre de bits de `value` mis à `1`
...
`count_bits(0)` doit retourner `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` doit retourner `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` doit retourner `4`
