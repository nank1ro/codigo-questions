...
Complétez `low_nibble` pour qu'elle retourne uniquement les quatre bits les plus bas de `value`
...
`low_nibble(0xAB)` doit retourner `11`

```c
    try_catch(low_nibble(0xAB) == 11);
```

`low_nibble(255)` doit retourner `15`

```c
    try_catch(low_nibble(255) == 15);
```

`low_nibble(16)` doit retourner `0`
