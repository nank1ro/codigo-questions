...
Complétez `is_bit_set` pour qu'elle retourne `1` quand le bit `n` de `value` est activé et `0` sinon, en supposant que `n` est plus petit que 32
...
`is_bit_set(10, 1)` doit retourner `1`

```c
    try_catch(is_bit_set(10, 1) == 1);
```

`is_bit_set(10, 2)` doit retourner `0`

```c
    try_catch(is_bit_set(10, 2) == 0);
```

`is_bit_set(10, 3)` doit retourner `1`
