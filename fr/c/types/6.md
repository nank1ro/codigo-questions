...
Complétez `truncate` pour qu'elle retourne la partie entière de `x` sous forme d'`int`, à l'aide d'une conversion explicite
...
`truncate(3.99)` doit retourner `3`

```c
    try_catch(truncate(3.99) == 3);
```

`truncate(-2.5)` doit retourner `-2`

```c
    try_catch(truncate(-2.5) == -2);
```

`truncate(7.0)` doit retourner `7`
