...
Complétez `to_bool` pour qu'elle retourne `false` quand `x` vaut exactement `0`, et `true` pour toute autre valeur
...
`to_bool(0.5)` doit retourner `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` doit retourner `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` doit retourner `true`
