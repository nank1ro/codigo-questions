...
Complétez `digit_value` pour qu'elle retourne le nombre représenté par le caractère chiffre `c` (par exemple `'7'` donne `7`)
...
`digit_value('7')` doit retourner `7`

```c
    try_catch(digit_value('7') == 7);
```

`digit_value('0')` doit retourner `0`

```c
    try_catch(digit_value('0') == 0);
```

`digit_value('9')` doit retourner `9`
