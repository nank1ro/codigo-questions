...
Complétez `to_celsius` pour qu'elle convertisse une température `f` de Fahrenheit en Celsius à l'aide de la formule `(f - 32) * 5 / 9`
...
`to_celsius(212)` doit retourner `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` doit retourner `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` doit retourner `10`
