...
Complétez `count_chars` pour qu'elle retourne le nombre de caractères de `text` en utilisant `strlen`
...
`count_chars("hello")` doit retourner `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` doit retourner `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` doit retourner `10`
