...
`area` retourne toujours `0` car son véritable calcul est mis en commentaire. Décommentez la ligne `return w * h;` et supprimez le placeholder `return 0;`
...
`area(3, 4)` doit retourner `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` doit retourner `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` doit retourner `0`
