...
`double_it` ne compile pas car le commentaire en fin de ligne de la première ligne n'est jamais fermé, donc il avale le `return`. Fermez le commentaire à la fin de cette même ligne
...
`double_it(4)` doit retourner `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` doit retourner `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` doit retourner `0`
