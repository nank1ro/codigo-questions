...
Accomplissez le travail décrit par le commentaire `TODO` dans `is_even`, puis supprimez le commentaire puisque le travail est fait
...
`is_even(4)` doit retourner `1`

```c
    try_catch(is_even(4) == 1);
```

`is_even(7)` doit retourner `0`

```c
    try_catch(is_even(7) == 0);
```

`is_even(0)` doit retourner `1`
