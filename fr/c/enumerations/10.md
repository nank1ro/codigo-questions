...
Complétez `next_color` pour qu'elle retourne la constante suivant `color`, en revenant de `BLUE` à `RED`. Il y a 3 couleurs, utilisez donc `% 3`
...
`next_color(RED)` doit retourner `GREEN`

```c
    try_catch(next_color(RED) == GREEN);
```

`next_color(GREEN)` doit retourner `BLUE`

```c
    try_catch(next_color(GREEN) == BLUE);
```

`next_color(BLUE)` doit retourner `RED`
