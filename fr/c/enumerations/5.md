...
Complétez `wheels` pour qu'elle retourne `2` pour `BIKE`, `4` pour `CAR` et `6` pour `TRUCK`, en utilisant un `switch` sur `vehicle`
...
`wheels(BIKE)` doit retourner `2`

```c
    try_catch(wheels(BIKE) == 2);
```

`wheels(CAR)` doit retourner `4`

```c
    try_catch(wheels(CAR) == 4);
```

`wheels(TRUCK)` doit retourner `6`
