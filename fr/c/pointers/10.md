...
Complète `sum_between` pour qu'elle renvoie la somme de tous les éléments depuis l'adresse `start` jusqu'à l'adresse `end` exclue
...
Les trois premiers éléments de `{1, 2, 3, 4}` doivent donner une somme de `6`
...
Les éléments d'indice `1` à `3` de `{5, 6, 7, 8}` doivent donner une somme de `21`
...
Un intervalle vide doit donner une somme de `0`
