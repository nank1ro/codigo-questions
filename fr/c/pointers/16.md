...
Complète `find_max` pour qu'elle renvoie un pointeur vers le plus grand des `size` éléments de `values`, ou `NULL` quand `size` vaut `0`
...
Dans `{3, 9, 4}`, le résultat doit pointer vers le `9`
...
La valeur lue via le résultat de `{-2, -8, -1}` doit être `-1`
