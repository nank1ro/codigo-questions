...
Complète `value_or` pour qu'elle renvoie la valeur vers laquelle `p` pointe, ou `fallback` quand `p` est `NULL`
...
En pointant vers `2.5`, le résultat doit être `2.5`
...
Avec un pointeur `NULL`, le résultat doit être la valeur de secours `-1.0`

```c
    try_catch(value_or(NULL, -1.0) == -1.0);
```

En pointant vers `0.0`, le résultat doit être `0.0`, pas la valeur de secours
