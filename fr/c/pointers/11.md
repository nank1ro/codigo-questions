...
Complète `swap` pour qu'elle échange les valeurs des deux variables vers lesquelles `a` et `b` pointent
...
Échanger `1` et `2` doit donner `2` et `1`
...
Échanger `-5` et `40` doit donner `40` et `-5`
...
Échanger deux fois doit restaurer les valeurs d'origine
