...
Complète `deposit` pour qu'elle ajoute `amount` au `balance` du compte pointé par `account`
...
Déposer `50` sur un solde de `100` doit donner `150`
...
Déposer `0` doit laisser le solde inchangé
...
Deux dépôts de `10` et `20` sur un compte vide doivent donner `30`
