...
Complète `make_node` pour qu'elle alloue un nœud sur le tas, y stocke `value`, mette `next` à `NULL` et renvoie son adresse
...
`make_node(5)` doit renvoyer un nœud dont la `value` est `5`
...
Le `next` d'un nouveau nœud doit être `NULL`
...
Deux appels doivent renvoyer deux nœuds différents
