...
Defina um método `__init__()` para a sua classe `Animal`.
Passe o argumento `self` por enquanto.
Por fim, coloque o `pass` no bloco de código da definição de `__init__()`
