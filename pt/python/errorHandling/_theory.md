...
try:
    ...
    raise
```
Escrever `raise ValueError(...)` em vez disso criaria uma nova exceção com seu próprio traceback — não seria mais a mesma falha que você capturou, e é exatamente essa identidade que um `raise` sozinho preserva.
