...
Complete `sign` para que retorne `1` quando `n` for positivo, `-1` quando for negativo e `0` quando for zero
...
`sign(9)` deve retornar `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` deve retornar `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` deve retornar `0`
