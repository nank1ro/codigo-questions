...
Complete `sum_between` para que ela retorne a soma de cada elemento do endereço `start` até, mas não incluindo, o endereço `end`
...
Os três primeiros elementos de `{1, 2, 3, 4}` devem somar `6`
...
Os elementos do índice `1` ao `3` de `{5, 6, 7, 8}` devem somar `21`
...
Um intervalo vazio deve somar `0`
