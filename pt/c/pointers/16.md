...
Complete `find_max` para que ela retorne um ponteiro para o maior dos `size` elementos em `values`, ou `NULL` quando `size` for `0`
...
Em `{3, 9, 4}` o resultado deve apontar para o `9`
...
O valor lido por meio do resultado de `{-2, -8, -1}` deve ser `-1`
