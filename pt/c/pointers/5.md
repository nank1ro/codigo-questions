...
Complete `value_or` para que ela retorne o valor para o qual `p` aponta, ou `fallback` quando `p` for `NULL`
...
Apontando para `2.5` o resultado deve ser `2.5`
...
Com um ponteiro `NULL` o resultado deve ser o fallback `-1.0`

```c
    try_catch(value_or(NULL, -1.0) == -1.0);
```

Apontando para `0.0` o resultado deve ser `0.0`, não o fallback
