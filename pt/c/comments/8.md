...
Complete o trabalho descrito pelo comentário `TODO` em `is_even` e depois remova o comentário, já que o trabalho está pronto
...
```c
int is_even(int n) {
    // TODO: retornar 1 quando n for par, 0 caso contrário
    return 0;
}
```
...
`is_even(4)` deve retornar `1`

```c
    try_catch(is_even(4) == 1);
```

`is_even(7)` deve retornar `0`

```c
    try_catch(is_even(7) == 0);
```

`is_even(0)` deve retornar `1`
