...
`area` sempre retorna `0` porque seu cálculo real está comentado. Descomente a linha `return w * h;` e exclua o `return 0;` provisório
...
`area(3, 4)` deve retornar `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` deve retornar `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` deve retornar `0`
