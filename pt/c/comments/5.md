...
`double_it` não compila porque o comentário de fim de linha na primeira linha nunca é fechado, então ele engole o `return`. Feche o comentário no fim dessa mesma linha
...
`double_it(4)` deve retornar `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` deve retornar `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` deve retornar `0`
