...
Complete `count_chars` para que ela retorne o número de caracteres em `text` usando `strlen`
...
`count_chars("hello")` deve retornar `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` deve retornar `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` deve retornar `10`
