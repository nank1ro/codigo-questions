...
Complete `power_of_two` para que retorne 2 elevado a `n`, usando um left shift em `1u` e assumindo que `n` é menor que 32
...
`power_of_two(0)` deve retornar `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` deve retornar `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` deve retornar `1024`
