...
Complete `count_bits` para que retorne quantos bits de `value` estão definidos como `1`
...
`count_bits(0)` deve retornar `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` deve retornar `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` deve retornar `4`
