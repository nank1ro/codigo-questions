...
Complete `make_node` para que ela aloque um nó no heap, guarde `value` nele, defina `next` como `NULL` e retorne seu endereço
...
`make_node(5)` deve retornar um nó cujo `value` é `5`
...
O `next` de um novo nó deve ser `NULL`
...
Duas chamadas devem retornar dois nós diferentes
