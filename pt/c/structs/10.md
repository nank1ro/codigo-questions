...
Complete `deposit` para que ela adicione `amount` ao `balance` da conta à qual `account` aponta
...
Depositar `50` em um saldo de `100` deve dar `150`
...
Depositar `0` deve deixar o saldo inalterado
...
Dois depósitos de `10` e `20` em uma conta vazia devem dar `30`
