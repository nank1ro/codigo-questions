...
Complete `can_add` para que ela retorne `1` quando `a + b` não exceder `INT_MAX` e `0` caso contrário (ambos os argumentos são não negativos)
...
`can_add(1, 2)` deve retornar `1`

```c
    try_catch(can_add(1, 2) == 1);
```

`can_add(INT_MAX, 1)` deve retornar `0`

```c
    try_catch(can_add(INT_MAX, 1) == 0);
```

`can_add(INT_MAX - 5, 5)` deve retornar `1`
