...
Complete `to_celsius` para que ela converta uma temperatura `f` de Fahrenheit para Celsius usando a fórmula `(f - 32) * 5 / 9`
...
`to_celsius(212)` deve retornar `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` deve retornar `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` deve retornar `10`
