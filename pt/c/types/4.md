...
Complete `average` para que ela retorne a média entre `a` e `b` como um `double`, mantendo a parte decimal
...
`average(3, 4)` deve retornar `3.5`

```c
    try_catch(average(3, 4) == 3.5);
```

`average(2, 2)` deve retornar `2`
