...
Complete `to_bool` para que ela retorne `false` quando `x` for exatamente `0` e `true` para qualquer outro valor
...
`to_bool(0.5)` deve retornar `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` deve retornar `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` deve retornar `true`
