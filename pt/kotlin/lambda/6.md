...
Escreva uma função `evenSquares(numbers: List<Int>): List<Int>` que retorne os quadrados dos números pares de `numbers`, na mesma ordem, encadeando `filter` e `map`.

Exemplo: `evenSquares(listOf(1, 2, 3, 4))` retorna `[4, 16]`.
...
`evenSquares(listOf(1, 2, 3, 4))` deve retornar `[4, 16]`

```kotlin
    tryCatch(evenSquares(listOf(1, 2, 3, 4)) == listOf(4, 16))
```

`evenSquares(listOf(1, 3, 5))` deve retornar uma lista vazia

```kotlin
    tryCatch(evenSquares(listOf(1, 3, 5)) == listOf<Int>())
```

`evenSquares(listOf(6, -2, 0))` deve retornar `[36, 4, 0]`

```kotlin
    tryCatch(evenSquares(listOf(6, -2, 0)) == listOf(36, 4, 0))
```

`evenSquares(listOf())` deve retornar uma lista vazia
