---
language: kotlin
exerciseType: 1
---

# --description--

Una classe `sealed` descrive un insieme **chiuso** di alternative: sono ammesse solo le sottoclassi scritte nello stesso package e modulo, così il compilatore le conosce tutte.

```kotlin
sealed class Shape
data class Circle(val radius: Int) : Shape()
data class Square(val side: Int) : Shape()
```

Il vantaggio è il **`when` esaustivo**: quando ramifichi su un tipo sealed e copri ogni sottoclasse, puoi omettere il ramo `else`. Se più avanti aggiungi una nuova sottoclasse, il compilatore segnala ogni `when` che hai dimenticato di aggiornare, invece di prendere silenziosamente l'`else`.

```kotlin
fun name(shape: Shape): String = when (shape) {
    is Circle -> "circle"
    is Square -> "square"
}
```

Dopo `is Circle` il valore viene sottoposto a smart cast, quindi `shape.radius` è disponibile dentro quel ramo senza alcun cast manuale.

# --instructions--

Dichiara una classe sealed `Shape` con due sottoclassi data class: `Square`, con una proprietà `val` `side` di tipo `Int`, e `Rectangle`, con le proprietà `val` `width` e `height` di tipo `Int`.

Poi scrivi una funzione `area(shape: Shape): Int` che restituisce l'area della figura, usando un `when` esaustivo senza ramo `else`.

# --before-seed--

```kotlin
// DO NOT EDIT FROM HERE
var _testFailedCount = 0;
var _testCount = 0;
fun tryCatch(assertion: Boolean) {
  _testCount++
    try { 
        if (!assertion) throw Exception()
    }
    catch (e: Throwable) {
        _testFailedCount++
        println("Test Case '--err-t$_testCount--' failed");
  }
};
// DO NOT EDIT UNTIL HERE
```

# --before-asserts--

```kotlin
fun main() {
```

# --asserts--

`area(Square(3))` must return `9`

```kotlin
    tryCatch(area(Square(3)) == 9)
```

`area(Rectangle(2, 5))` must return `10`

```kotlin
    tryCatch(area(Rectangle(2, 5)) == 10)
```

`area(Square(0))` must return `0`

```kotlin
    tryCatch(area(Square(0)) == 0)
```

`Square` and `Rectangle` must be subclasses of `Shape`

```kotlin
    val shapes: List<Shape> = listOf(Square(4), Rectangle(3, 3))
    tryCatch(shapes.map { area(it) } == listOf(16, 9))
```

# --after-asserts--

```kotlin
// DO NOT EDIT FROM HERE 
    println("Executed $_testCount tests, with $_testFailedCount failures");
}
// DO NOT EDIT UNTIL HERE
```

# --solutions--

```kotlin
sealed class Shape
data class Square(val side: Int) : Shape()
data class Rectangle(val width: Int, val height: Int) : Shape()

fun area(shape: Shape): Int = when (shape) {
    is Square -> shape.side * shape.side
    is Rectangle -> shape.width * shape.height
}
```
