...
Completa `sum_at` in modo che restituisca la somma dei valori a cui `a` e `b` puntano
...
Puntando a `3` e `4` la somma deve essere `7`
...
Puntando a `10` e `-10` la somma deve essere `0`
...
Passando due volte lo stesso indirizzo il suo valore deve raddoppiare
