...
Completa `find_max` in modo che restituisca un puntatore al più grande dei `size` elementi di `values`, oppure `NULL` quando `size` è `0`
...
In `{3, 9, 4}` il risultato deve puntare al `9`
...
Il valore letto attraverso il risultato di `{-2, -8, -1}` deve essere `-1`
