...
Completa `sign` in modo che restituisca `1` quando `n` è positivo, `-1` quando è negativo e `0` quando è zero
...
`sign(9)` deve restituire `1`

```c
    try_catch(sign(9) == 1);
```

`sign(-4)` deve restituire `-1`

```c
    try_catch(sign(-4) == -1);
```

`sign(0)` deve restituire `0`
