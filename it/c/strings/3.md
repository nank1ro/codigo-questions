...
Completa `count_chars` in modo che restituisca il numero di caratteri in `text` usando `strlen`
...
`count_chars("hello")` deve restituire `5`

```c
    try_catch(count_chars("hello") == 5);
```

`count_chars("")` deve restituire `0`

```c
    try_catch(count_chars("") == 0);
```

`count_chars("Codigo App")` deve restituire `10`
