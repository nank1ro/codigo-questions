...
Completa `digit_value` in modo che restituisca il numero rappresentato dal carattere cifra `c` (per esempio `'7'` dà `7`)
...
`digit_value('7')` deve restituire `7`

```c
    try_catch(digit_value('7') == 7);
```

`digit_value('0')` deve restituire `0`

```c
    try_catch(digit_value('0') == 0);
```

`digit_value('9')` deve restituire `9`
