...
Completa `truncate` in modo che restituisca la parte intera di `x` come `int`, usando un cast esplicito
...
`truncate(3.99)` deve restituire `3`

```c
    try_catch(truncate(3.99) == 3);
```

`truncate(-2.5)` deve restituire `-2`

```c
    try_catch(truncate(-2.5) == -2);
```

`truncate(7.0)` deve restituire `7`
