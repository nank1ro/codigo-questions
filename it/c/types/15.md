...
Completa `square` in modo che restituisca `n * n` come `long`, senza andare in overflow per valori grandi di `n`
...
`square(100000)` deve restituire `10000000000`

```c
    try_catch(square(100000) == 10000000000L);
```

`square(-2)` deve restituire `4`
