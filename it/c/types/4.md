...
Completa `average` in modo che restituisca la media di `a` e `b` come `double`, mantenendo la parte decimale
...
`average(3, 4)` deve restituire `3.5`

```c
    try_catch(average(3, 4) == 3.5);
```

`average(2, 2)` deve restituire `2`
