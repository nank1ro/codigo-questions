...
Completa `to_bool` in modo che restituisca `false` quando `x` è esattamente `0` e `true` per qualsiasi altro valore
...
`to_bool(0.5)` deve restituire `true`

```c
    try_catch(to_bool(0.5) == true);
```

`to_bool(0.0)` deve restituire `false`

```c
    try_catch(to_bool(0.0) == false);
```

`to_bool(-3.0)` deve restituire `true`
