...
Completa `to_celsius` in modo che converta una temperatura `f` da Fahrenheit a Celsius usando la formula `(f - 32) * 5 / 9`
...
`to_celsius(212)` deve restituire `100`

```c
    try_catch(to_celsius(212) == 100);
```

`to_celsius(32)` deve restituire `0`

```c
    try_catch(to_celsius(32) == 0);
```

`to_celsius(50)` deve restituire `10`
