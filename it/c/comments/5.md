...
`double_it` non compila perché il commento a fine riga sulla prima riga non viene mai chiuso, quindi ingoia il `return`. Chiudi il commento alla fine di quella stessa riga
...
```c
int double_it(int n) {
    int result = n * 2; /* il doppio dell'input
    return result;
}
```
...
`double_it(4)` deve restituire `8`

```c
    try_catch(double_it(4) == 8);
```

`double_it(-3)` deve restituire `-6`

```c
    try_catch(double_it(-3) == -6);
```

`double_it(0)` deve restituire `0`
