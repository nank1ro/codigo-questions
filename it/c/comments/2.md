...
`area` restituisce sempre `0` perché il suo calcolo vero e proprio è commentato. Decommenta la riga `return w * h;` ed elimina il segnaposto `return 0;`
...
`area(3, 4)` deve restituire `12`

```c
    try_catch(area(3, 4) == 12);
```

`area(5, 5)` deve restituire `25`

```c
    try_catch(area(5, 5) == 25);
```

`area(7, 0)` deve restituire `0`
