...
Completa il lavoro descritto dal commento `TODO` in `is_even`, poi rimuovi il commento dato che il lavoro è fatto
...
```c
int is_even(int n) {
    // TODO: restituisci 1 quando n è pari, altrimenti 0
    return 0;
}
```
...
`is_even(4)` deve restituire `1`

```c
    try_catch(is_even(4) == 1);
```

`is_even(7)` deve restituire `0`

```c
    try_catch(is_even(7) == 0);
```

`is_even(0)` deve restituire `1`
