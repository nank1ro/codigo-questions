...
```c
    try_catch(strcmp(due_per_uno(""), "Uno per te, uno per me.") == 0);
```

Dividi con "Daniele"

```c
    try_catch(strcmp(due_per_uno("Daniele"), "Uno per Daniele, uno per me.") == 0);
```

Dividi con "Marta"

```c
    try_catch(strcmp(due_per_uno("Marta"), "Uno per Marta, uno per me.") == 0);
```
...
char* due_per_uno(char* nome) {
    ...
    sprintf(result, "Uno per %s, Uno per me.", nome);
    return result;
}
```
