...
Completa `wheels` in modo che restituisca `2` per `BIKE`, `4` per `CAR` e `6` per `TRUCK`, usando uno `switch` su `vehicle`
...
`wheels(BIKE)` deve restituire `2`

```c
    try_catch(wheels(BIKE) == 2);
```

`wheels(CAR)` deve restituire `4`

```c
    try_catch(wheels(CAR) == 4);
```

`wheels(TRUCK)` deve restituire `6`
