...
Completa `is_bit_set` in modo che restituisca `1` quando il bit `n` di `value` è acceso e `0` altrimenti, supponendo che `n` sia minore di 32
...
`is_bit_set(10, 1)` deve restituire `1`

```c
    try_catch(is_bit_set(10, 1) == 1);
```

`is_bit_set(10, 2)` deve restituire `0`

```c
    try_catch(is_bit_set(10, 2) == 0);
```

`is_bit_set(10, 3)` deve restituire `1`
