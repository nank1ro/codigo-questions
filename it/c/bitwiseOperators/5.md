...
Completa `power_of_two` in modo che restituisca 2 elevato a `n`, usando uno scorrimento a sinistra di `1u` e supponendo che `n` sia minore di 32
...
`power_of_two(0)` deve restituire `1`

```c
    try_catch(power_of_two(0) == 1);
```

`power_of_two(3)` deve restituire `8`

```c
    try_catch(power_of_two(3) == 8);
```

`power_of_two(10)` deve restituire `1024`
