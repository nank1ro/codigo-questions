...
Completa `count_bits` in modo che restituisca quanti bit di `value` sono impostati a `1`
...
`count_bits(0)` deve restituire `0`

```c
    try_catch(count_bits(0) == 0);
```

`count_bits(255)` deve restituire `8`

```c
    try_catch(count_bits(255) == 8);
```

`count_bits(0xA5)` deve restituire `4`
